% Search for uavsimblockset in subfolders and a few levels up
function longcall(filename, callerpath, nUplevels, varargin)
% Always start by using caching, then fall back to search if file can't be
% found:
usecaching = true;

% Go up a couple of levels to recursively search from there:
updir = '../';
for k=1:nUplevels
    updir = [updir '../'];
end
cd(updir);
tic;
disp([mfilename '>> Attempting (using caching) to find ' filename ' from ' pwd]);
args{1} = pwd;
args{2} = filename;
a = cachedcall(@subdir_cached, args);
if isempty(a)
    disp([mfilename '>> could not find file ' filename ' I was supposed to run in cache, trying without caching ...']);
    a = subdir(filename);
end
Dt = toc;
...
if isempty(a)
    disp([mfilename '>> could not find file ' filename ' I was supposed to run...']);
elseif length(a) == 1
    % Call:
    disp([mfilename '>> Found file ' filename ' after ' num2str(Dt) ' s']);
    run(a.name)
else
    disp([mfilename '>> There is more than one file called ' filename, ', aborting']);
end
% Back to caller path:
cd(callerpath);

% Wrapper function that includes the call path to make sure that cachedcall
% only returns cached results when searching from the same folder.
function out = subdir_cached(path, fname)
out = subdir(fname);
